...
function InfoRow({ icon: Icon, label, value, accent }: { icon: typeof Hash; label: string; value: string; accent: string }) {
  return (
...
function InfoCard({ title, icon: Icon, accent, children }: { title: string; icon: typeof Hash; accent: string; children: React.ReactNode }) {
  return (
...
function RecipeList({ items, emptyLabel }: { items: ReturnType<typeof mockUserRecipes>; emptyLabel: string }) {
  ...
  return (
    <div className="space-y-2">
      {items.map((r, i) => {
        const color = isDark ? CATEGORICAL[r.colorIndex].dark : CATEGORICAL[r.colorIndex].light;
        return (
          <div
            key={i}
          ...
          >
            <div className="w-10.5 h-10.5 rounded-xl flex items-center justify-center shrink-0" style={{ width: 42, height: 42, backgroundColor: `${color}1F` }}>
...
export default function AdminUserDetailPage() {
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5 mt-1.5">
            <span
              ...
              style={{ backgroundColor: user.role === "admin" ? `${accent}1F` : "#10B98119", color: user.role === "admin" ? accent : "#10B981" }}
            >
            ...
            <span
              className="flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full"
              style={{ backgroundColor: "var(--tm-subtle)", color: user.isActive ? "#10B981" : "#F43F5E" }}
            >
              <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: user.isActive ? "#10B981" : "#F43F5E" }} />
      ...
      {tab === "profile" && (
        <div className="space-y-3">
          <InfoCard title="Account" icon={UserIcon} accent={accent}>
            ...
            <InfoRow icon={CalendarDays} label="Joined" value={user.createdAt} accent={accent} />
          </InfoCard>
          ...
          <InfoCard title="Nutrition Goals" icon={Flame} accent={accent}>
            ...
            <InfoRow icon={Flame} label="Calorie Target" value={user.calorieTarget ? `${user.calorieTarget} kcal/day` : "—"} accent={accent} />
            <InfoRow icon={Flag} label="Primary Goal" value={user.primaryGoal ?? "—"} accent={accent} />
          </InfoCard>
            ...
            {user.dietaryRestrictions.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-1.5 pt-2">
                {user.dietaryRestrictions.map((tagName) => (
                  <span key={tagName} className="text-[11px] font-semibold px-2.5 py-1 rounded-full" style={{ backgroundColor: `${accent}1A`, color: accent }}>
                    {tagName}
                  </span>
                ))}
              </div>
      ...
      {tab === "saved" && <RecipeList items={mockUserRecipes(user, "saved")} emptyLabel="No saved recipes yet" />}
      {tab === "recipes" && <RecipeList items={mockUserRecipes(user, "created")} emptyLabel="No recipes created yet" />}
