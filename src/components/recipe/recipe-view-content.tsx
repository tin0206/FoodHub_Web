...
export function ingredientLines(recipe: ApiRecipe): string[] {
  ...
  if (recipe.mapped_ingredients?.length) {
    return recipe.mapped_ingredients.map(
      (m) =>
        `${formatAmount(m.amount)} ${m.unit} ${m.natural_name || m.mapped_name}`,
    );
  }
...
export function NutritionBlock({
  ...
  const extraEntries = Object.entries(perServing).filter(
    ([key]) => !CORE_NUTRITION_KEYS.includes(key),
  );
  ...
  return (
    <div className="lg:grid lg:grid-cols-[minmax(0,380px)_1fr] lg:gap-6 lg:items-start">
      ...
      <div className="lg:sticky lg:top-0">
        ...
        <div
          className="flex items-center gap-4 mb-3 lg:mb-0 text-sm flex-wrap"
          style={{ color: "var(--tm-text-2)" }}
        >
          ...
          {(secondaryStat === "servings" || secondaryStat === "all") &&
            recipe.estimated_servings != null && (
              <span className="flex items-center gap-1.5">
                <Users size={14} color={theme.start} />{" "}
                {recipe.estimated_servings} {t.servingsSuffix}
              </span>
            )}
          {(secondaryStat === "calories" || secondaryStat === "all") && (
            <span className="flex items-center gap-1.5">
              <Flame size={14} color={theme.start} /> {meta.calories}{" "}
              {t.calSuffix}
            </span>
          )}
        </div>
